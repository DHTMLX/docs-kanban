...
title: update-comment 事件 API
description: 阅读 DHTMLX Kanban 的 update-comment 事件 API。了解如何处理更新评论时触发的事件。
---
...
- `comment` - （必需）要更新的评论的配置对象。您可以在此指定以下参数：
    ...
    - `text` - （可选）已更新评论的文本内容
    - `date` - （可选）已更新评论的日期
    - `html` - （可选）已更新评论的 HTML 标记。启用 [`editorShape`](/api/config/js_kanban_editorshape_config/#parameters-for-comments-type) 配置中的 `html` 属性，以显示 HTML 标记而非纯文本
