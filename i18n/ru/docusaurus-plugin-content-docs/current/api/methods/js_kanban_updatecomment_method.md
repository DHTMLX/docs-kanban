...
- `comment` - (обязательно) объект конфигурации обновляемого комментария. Здесь можно указать следующие параметры:
    ...
    - `html` - (необязательно) HTML-разметка обновляемого комментария. Включите свойство `html` в конфигурации [`editorShape`](/api/config/js_kanban_editorshape_config/#parameters-for-comments-type), чтобы отображать HTML-разметку вместо текста
...
**Журнал изменений:** Метод добавлен в v1.4