...
Метод возвращает карту действий по умолчанию, которая используется внутри `RestDataProvider`. Переопределите этот метод, чтобы добавить собственные обработчики или заменить стандартные на пользовательскую логику.
