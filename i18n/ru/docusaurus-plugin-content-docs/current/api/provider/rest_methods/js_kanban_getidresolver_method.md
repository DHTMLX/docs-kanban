...
Чаще всего этот метод нужен при подключении `RestDataProvider` к `kanbanUpdates` в многопользовательской конфигурации, чтобы серверные события, адресованные серверным ID, корректно применялись к сущностям, которые ещё существуют на клиенте под временными ID:
