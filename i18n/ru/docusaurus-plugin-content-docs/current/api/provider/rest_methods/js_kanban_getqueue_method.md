...
Очередь используется внутри другими методами `RestDataProvider`. Чаще всего очередь задействуется косвенно, через [`getIDResolver()`](api/provider/rest_methods/js_kanban_getidresolver_method.md), который построен на логике разрешения идентификаторов из очереди.
