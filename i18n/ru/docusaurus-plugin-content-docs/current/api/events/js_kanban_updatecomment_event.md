...
- `comment` - (обязательный) объект конфигурации обновляемого комментария. Здесь можно указать следующие параметры:
    ...
    - `html` - (необязательный) HTML-разметка обновлённого комментария. Включите свойство `html` в конфиге [`editorShape`](/api/config/js_kanban_editorshape_config/#parameters-for-comments-type), чтобы отображать HTML-разметку вместо текста
...
**Журнал изменений:** Событие добавлено в v1.4