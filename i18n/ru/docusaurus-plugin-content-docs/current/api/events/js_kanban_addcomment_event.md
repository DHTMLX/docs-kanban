...
- `comment` - (обязательный) объект конфигурации нового комментария. Здесь можно указать следующие параметры:
    ...
    - `html` - (необязательный) HTML-разметка нового комментария. Включите свойство `html` в конфигурации [`editorShape`](/api/config/js_kanban_editorshape_config/#parameters-for-comments-type), чтобы отображать HTML-разметку вместо текста
