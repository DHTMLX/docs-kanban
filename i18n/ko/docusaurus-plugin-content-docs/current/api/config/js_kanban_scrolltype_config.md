...
`renderType: "lazy"`와 [`scrollType: "column"`](api/config/js_kanban_scrolltype_config.md)을 함께 사용할 때에도 카드의 높이를 고정하는 것이 좋습니다. 이 레이아웃 유형에서는 카드의 가변 높이가 지원되지만, 커스텀 카드 콘텐츠와 함께 사용할 경우 안정적으로 동작하지 않을 수 있습니다.
:::
